...
def train():
    ...
    current_classing_valid_out_file = result_dir + '/intent.valid.hyp.txt'
    current_classing_test_out_file = result_dir + '/intent.test.hyp.txt'
        ...
        while model.global_step.eval() < FLAGS.max_training_steps:
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                    ...
                    for bucket_id in xrange(len(_buckets)):
                        ...
                        for i in xrange(len(data_set[bucket_id])):
                            ...
                            if task['tagging'] == 1:
                                ...
                                hyp_tag_list.append([rev_tag_vocab[np.argmax(x)] for x in \
                                                     tagging_logits[:sequence_length[0]]])

                    accuracy = float(correct_count) * 100 / count
                    if task['intent'] == 1:
                        if mode == 'Eval':
                            class_out_file = current_classing_valid_out_file
                        elif mode == 'Test':
                            class_out_file = current_classing_test_out_file
                        save_class_file(hyp_label_list, ref_label_list, class_out_file)
                ...
                if task['tagging'] == 1 \
                    ...
                    cmd = 'copy "%s" "%s"' % (current_taging_valid_out_file,
                                              current_taging_valid_out_file[:-4] + '.best_f1_%.2f.txt' % best_valid_score)
                    cmd = cmd.replace('/', '\\')
                    subprocess.call(cmd, shell=True)
                # test, run test after each validation for development purpose.

                if task['intent'] == 1 and valid_accuracy > best_valid_accuracy:
                    best_valid_accuracy = valid_accuracy
                    # save the best output file
                    cmd = 'copy "%s" "%s"' % (current_classing_valid_out_file,
                                              current_classing_valid_out_file[:-4] + '.best_acc_%.2f.txt' % best_valid_accuracy)
                    cmd = cmd.replace('/', '\\')
                    subprocess.call(cmd, shell=True)
                ...
                if task['tagging'] == 1 \
                    ...
                    # save the best output file
                    cmd = 'copy "%s" "%s"' % (current_taging_test_out_file,
                                              current_taging_test_out_file[:-4] + '.best_f1_%.2f.txt' % best_test_score)
                    cmd = cmd.replace('/', '\\')
                    subprocess.call(cmd, shell=True)

                if task['intent'] == 1 and test_accuracy > best_test_accuracy:
                    best_test_accuracy = test_accuracy
                    # save the best output file
                    cmd = 'copy "%s" "%s"' % (current_classing_test_out_file,
                                              current_classing_test_out_file[:-4] + '.best_acc_%.2f.txt' % best_test_accuracy)
                    cmd = cmd.replace('/', '\\')
                    subprocess.call(cmd, shell=True)
